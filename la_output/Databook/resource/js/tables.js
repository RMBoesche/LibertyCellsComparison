...
function buildRefTrgLinks(tbl) {
  ...
  if (!pageInfo)
    return "";
  var refCellName = pageInfo.getAttribute('RefCellName');
  var trgCellName = pageInfo.getAttribute('TrgCellName');
  var refNode = pageInfo.getAttribute('RefNode');
  var trgNode = pageInfo.getAttribute('TrgNode');
  var fileExt = pageInfo.getAttribute('FileExt');
  if (tbl.getAttribute('rid')) {
    table += "<br/><a href=\"" + refCellName + "_" + refNode + fileExt + "#" + TABLE_ID + tbl.getAttribute('rid') + "\">ref</a>  |  ";
    table += "<a href=\"" + trgCellName + "_" + trgNode + fileExt + "#" + TABLE_ID + tbl.getAttribute('tid') + "\">trg</a>";
  }
  return table;
}
