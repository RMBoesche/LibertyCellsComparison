    ...
    for key, luts in lut_info.items():
        ...
        for lut in luts:
            ...
            for i, row in enumerate(values):
                ...
                print(f"Plotting {key} for {cell_name} with index_1: {index_1} and row_values: {row_values}")
                if len(index_1) == len(row_values):  # Ensure both lists have the same length
                    plt.plot(index_1, row_values, label=f'{index_2_label} = {index_2[i]}')
                else:
        ...
        plt.xlabel(index_1_label.replace('_', ' ').title())
        plt.ylabel(key.replace('_', ' ').title())
        plt.title(f'{cell_name} - {key.replace("_", " ").title()} vs {index_1_label.replace("_", " ").title()}')
        plt.legend(title=index_2_label.replace('_', ' ').title())
        plt.grid(True)
        plt.savefig(f'{cell_name}_{key}_index1.png')  # Save the plot as a PNG file
        plt.close()
        ...
        for lut in luts:
            ...
            for i, col in enumerate(zip(*values)):  # Transpose the rows to columns for index_2
                ...
                print(f"Plotting {key} for {cell_name} with index_2: {index_2} and col_values: {col_values}")
                if len(index_2) == len(col_values):  # Ensure both lists have the same length
                    plt.plot(index_2, col_values, label=f'{index_1_label} = {index_1[i]}')
                else:
        ...
        plt.xlabel(index_2_label.replace('_', ' ').title())
        plt.ylabel(key.replace('_', ' ').title())
        plt.title(f'{cell_name} - {key.replace("_", " ").title()} vs {index_2_label.replace("_", " ").title()}')
        plt.legend(title=index_1_label.replace('_', ' ').title())
        plt.grid(True)
        plt.savefig(f'{cell_name}_{key}_index2.png')  # Save the plot as a PNG file
        plt.close()
