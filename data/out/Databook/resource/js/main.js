...
function readPlot() {
  ...
  var title;
  try {
    title = decodeURIComponent(params[1]);
  } catch (e) {
    title = params[1];
  }
  var context;
  try {
    context = decodeURIComponent(params[2]);
  } catch (e) {
    context = params[2];
  }
  ...
  if (str == 'none') {
    ...
    var tableHtml;
    try {
      tableHtml = decodeURIComponent(params[4]);
    } catch (e) {
      tableHtml = params[4];
    }

    window.document.body.innerHTML += context + '<br/>' + tableHtml;
    createTables();
    createToolTips();
...
function readPlotList() {
  ...
  baseHtml = '<table class="navigation" width="100%">' +
             ...
             '    <td align="right">' +
             '      <a href="http://www.silvaco.com" target="_blank"><img src="../image/Silvacologo_small.png" border="0"/></a>' +
             '      <br/><br/>' +
             '      <span class="comment">Copyright ® 1984-' + curYear + ' Silvaco, Inc. All rights reserved.</span>' +
             '    </td>' +
             '  </tr>';
...
function unsupportedBrowserMsg() {
  alert("Your browser does not support this feature. Please consider using one of the Silvaco Liberty Analyzer recommended browsers.");
}
